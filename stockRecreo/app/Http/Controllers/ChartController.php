...
class ChartController extends Controller
{
  public function graf1()
    ...
    $ventas_del_dia = DB::table('ventas')->select(DB::raw('created_at as fecha, monto'))->whereBetween('created_at', array($fechaactual, $fechaactual.' 23:59:59'))->orderBy('created_at','ASC')->get();
...
//4 productos mas vendidos
    $top_mas_vendidos=DB::table('ventas')->join('detalle_ventas','ventas.id','=','detalle_ventas.id_venta')->join('productos','productos.id','=','detalle_ventas.id_producto')->select(DB::raw('productos.nombre as nombre, SUM( detalle_ventas.cantidad)as num_prod'))->whereBetween('ventas.created_at', array($fecha_30_Dias_antes , $fechaactual.' 23:59:59'))->groupBy('nombre')->orderBy('num_prod','ASC')->LIMIT('5')->get(); 
	...
	$prod_vendidos_en_periodo= DB::table('ventas')->select(DB::raw('DATE(created_at) as fecha, count(*) as Ventas'))->whereBetween('created_at', array($fecha_30_Dias_antes , $fechaactual.' 23:59:59'))->groupBy('fecha')->orderBy('fecha','ASC')->get();
